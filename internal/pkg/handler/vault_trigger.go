...
func RegisterVaultEndpoint(collectors metrics.Collectors) {
	...
	http.HandleFunc("/trigger/vault", func(w http.ResponseWriter, r *http.Request) {
		...
		limitedReader := http.MaxBytesReader(w, r.Body, 1<<20)
		...
		if !isValidVaultPath(payload.Path) {
			logrus.Warnf("vault trigger: rejected invalid path from %s", r.RemoteAddr)
		...
		failures := 0
		sanitizedPath := sanitizePath(payload.Path)
		...
		for _, ns := range namespaces {
			...
			if options.WebhookUrl != "" {
				...
				if err := sendUpgradeWebhook(cfg, options.WebhookUrl); err != nil {
					logrus.Errorf("Vault trigger webhook failed for path '%s' ns '%s': %v", sanitizedPath, ns, sanitizeError(err))
					failures++
			...
			if err := doRollingUpgrade(cfg, collectors, nil, invokeReloadStrategy); err != nil {
				logrus.Errorf("Vault trigger upgrade failed for path '%s' ns '%s': %v", sanitizedPath, ns, sanitizeError(err))
				failures++
				collectors.VaultTriggers.WithLabelValues("false").Inc()
				collectors.VaultTriggersByNamespace.WithLabelValues("false", ns).Inc()
			} else {
